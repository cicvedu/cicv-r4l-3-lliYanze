...
impl E1000Ops {
    ...
    pub(crate) fn e1000_reset_hw(&self) -> Result {
        ...
        self.mem_addr
            .writel(manc & (!E1000_MANC_ARP_EN), E1000_MANC)?;
    ...
    fn e1000_configure_tx(&self, tx_ring: &TxRingBuf) -> Result {
        ...
        self.mem_addr
            .writel((TX_RING_SIZE * 16) as u32, E1000_TDLEN)?;
        // set ring buf start address
        self.mem_addr
            .writel(tx_ring.desc.get_dma_addr() as u32, E1000_TDBAL)?;
        self.mem_addr.writel(0, E1000_TDBAH)?;

        let tctl =
            (E1000_TCTL_EN | E1000_TCTL_PSP | 0x10 << E1000_CT_SHIFT | 0x40 << E1000_COLD_SHIFT);
        self.mem_addr.writel(tctl, E1000_TCTL)?;

        let tipg = (DEFAULT_82543_TIPG_IPGT_COPPER
    ...
    fn e1000_configure_rx(&self, rx_ring: &RxRingBuf) -> Result {
        ...
        self.mem_addr
            .writel((RX_RING_SIZE * 16) as u32, E1000_RDLEN)?;
        self.mem_addr
            .writel(rx_ring.desc.get_dma_addr() as u32, E1000_RDBAL)?;
        self.mem_addr.writel(0, E1000_RDBAH)?;

        let rctl = (E1000_RCTL_EN | E1000_RCTL_BAM | E1000_RCTL_SZ_2048 | E1000_RCTL_SECRC);
