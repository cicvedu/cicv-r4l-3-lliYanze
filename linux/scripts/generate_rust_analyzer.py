...
def generate_crates(srctree, objtree, sysroot_src, external_src):
    ...
    crates[-1]["source"] = {
        "include_dirs": [str(srctree / "rust" / "kernel"), str(objtree / "rust")],
        "exclude_dirs": [],
    }
    ...
    for folder in ("samples", "drivers") if external_src is None else [external_src]:
        for path in (srctree / folder).rglob("*.rs"):
            ...
            if is_root_crate(path.parent / "Makefile", f"{name}.o") or is_root_crate(
                path.parent / "Kbuild", f"{name}.o"
            ):
...
def main():
    ...
    rust_project = {
        "crates": generate_crates(
            args.srctree, args.objtree, args.sysroot_src, args.exttree
        ),
